...
2.  **Initialization**: Implement platform-specific startup and mandatory I/O (`io_get`, `io_put`, `io_read_record`, `io_end_record`, `io_end_field`, `io_save`, `io_load`) in its own directory. On failure, I/O routines should invoke `raise ERR_IO_ERROR`.
