...
void call_find_name(const char* name, const char* name_table, char expect_index,
    const char* expect_node_ptr, int line) {        
    char index;
    ...
    ASSERT_PTR_EQ(node_ptr, expect_node_ptr);
}
...
void call_find_name_fail(const char* name, const char* name_table, char expect_index,
    ...
    ASSERT_NE(err, 0);
    ASSERT_EQ(index, expect_index);
    // On fail node_ptr should always point to 0 at the end of the name table.
    ASSERT_PTR_EQ(node_ptr, expect_node_ptr);
}
...
void test_add_variable(void) {
    ...
    ASSERT_PTR_EQ(node_ptr, variable_name_table_ptr + 2);
    ...
    ASSERT_PTR_EQ(node_ptr, variable_name_table_ptr + 4 + 4);
