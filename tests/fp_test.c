...
void test_normalize(void) {
    ...
    call_normalize(POSITIVE, 129, 0, 0, 0, 0, 0, __LINE__);
    // 1
    call_normalize(POSITIVE, 129, 0x00, 0x00000001, 0x00, 98, 0x80000000, __LINE__);
    // -1
    call_normalize(NEGATIVE, 129, 0x00, 0x00000001, 0x00, 98, 0x80000000, __LINE__);
    // 32,767
    call_normalize(POSITIVE, 159, 0x00, 0x00007FFF, 0x00, 142, 0xFFFE0000, __LINE__);
    // 2,147,483,647
    call_normalize(POSITIVE, 160, 0x00, 0x7FFFFFFF, 0x00, 159, 0xFFFFFFFE, __LINE__);
    // -2,147,483,648
    call_normalize(NEGATIVE, 160, 0x00, 0x80000000, 0x00, 160, 0x80000000, __LINE__);
    // 2,286,166,545
    call_normalize(POSITIVE, 159, 0x00, 0x88442211, 0x00, 159, 0x88442211, __LINE__);
    // 4,294,967,296
    call_normalize(POSITIVE, 160, 0x01, 0x00000000, 0x00, 161, 0x80000000, __LINE__);
...
const ComparisonTestCase comparison_test_cases[] = {
    ...
    // -1 <=> 0
    { { 0x80000000, 128 }, { 0x00000000,   0 }, -1 },
    // 0 <=> -1
    { { 0x00000000,   0 }, { 0x80000000, 128 },  1 },
    // -1 <=> -1
    { { 0x80000000, 128 }, { 0x80000000, 128 },  0 },
    // -2 <=> -1
    { { 0x80000000, 129 }, { 0x80000000, 128 }, -1 },
    // -1 <=> -2
    { { 0x80000000, 128 }, { 0x80000000, 129 },  1 },
    // 1 <=> -1
    { { 0x00000000, 128 }, { 0x80000000, 128 },  1 },
    // -1 <=> 1
    { { 0x80000000, 128 }, { 0x00000000, 128 }, -1 },
    // -2 <=> -0.5
    { { 0x80000000, 129 }, { 0x80000000, 127 }, -1 },
    // -0.5 <=> -2
    { { 0x80000000, 127 }, { 0x80000000, 129 },  1 },
};
...
void call_string_to_fp(const char* input, unsigned long expect_t, char expect_e, char expect_y, int line) {
    Float expect_result;
    Float result;
...
void fail_string_to_fp(const char* input, int line) {
    fprintf(stderr, "  %s:%d: string_to_fp(\"%s\")\n", __FILE__, line, input);
    strcpy(buffer, input);
    string_to_fp(buffer, 0);
    ASSERT_NE(err, 0);
...
void test_string_to_fp(void) {
    ...
    // 0
    call_string_to_fp("0", 0x00000000, 0, 1, __LINE__);
    // 1
    call_string_to_fp("1", 0x00000000, 128, 1, __LINE__);
    // -1
    call_string_to_fp("-1", 0x80000000, 128, 2, __LINE__);
    // +1
    call_string_to_fp("+1", 0x00000000, 128, 2, __LINE__);
    // 10
    call_string_to_fp("10", 0x20000000, 131, 2, __LINE__);
    // 25
    call_string_to_fp("25", 0x48000000, 132, 2, __LINE__);
    // 100
    call_string_to_fp("100", 0x48000000, 134, 3, __LINE__);
    // -100
    call_string_to_fp("-100", 0xC8000000, 134, 4, __LINE__);
    // 3.14159
    call_string_to_fp("3.14159", 0x490FCF81, 129, 7, __LINE__);
    // 0.0314159
    call_string_to_fp("0.0314159", 0x00ADF571, 123, 9, __LINE__);
    // 2,147,483,647
    call_string_to_fp("2147483647", 0x7FFFFFFE, 158, 10, __LINE__);
    // -2,147,483,648
    call_string_to_fp("-2147483648", 0x80000000, 159, 11, __LINE__);
    // 1.025
    call_string_to_fp("1.025", 0x03333333, 128, 5, __LINE__);
    // log(2)                                         
    call_string_to_fp("0.693147181", 0x317217FA, 127, 11, __LINE__);
    // sqrt(2)                                         
    call_string_to_fp("1.414213562", 0x3504F333, 128, 11, __LINE__);
    // 10,000
    call_string_to_fp("1E5", 0x43500000, 144, 3, __LINE__);
    // 10,000 (alternate)
    call_string_to_fp("10E4", 0x43500000, 144, 4, __LINE__);
    // -10,000
    call_string_to_fp("-1E5", 0xC3500000, 144, 4, __LINE__);
    // 3.14159
    call_string_to_fp("31415.9E-4", 0x490FCF81, 129, 10, __LINE__);
    // 3.14159
    call_string_to_fp(".0314159E2", 0x490FCF81, 129, 10, __LINE__);
    // 1
    call_string_to_fp("1E0", 0x00000000, 128, 3, __LINE__);
    // -1
    call_string_to_fp("-1E0", 0x80000000, 128, 4, __LINE__);
    // 0
    call_string_to_fp("0E5", 0x00000000, 0, 3, __LINE__);
    // 0.1
    call_string_to_fp("100E-2", 0x00000000, 128, 6, __LINE__);
    // 1
    call_string_to_fp("1E", 0x00000000, 128, 2, __LINE__);

    // Verify that string_to_fp stops on non-digit.
    call_string_to_fp("10X", 0x20000000, 131, 2, __LINE__);
    call_string_to_fp("-100-", 0xC8000000, 134, 4, __LINE__);

    // Verify that string_to_fp handles characters with EOT set.
    call_string_to_fp("10\xB0", 0x48000000, 134, 3, __LINE__);
    call_string_to_fp("100\xAE", 0x48000000, 134, 4, __LINE__);
    call_string_to_fp("1E\xB5", 0x43500000, 144, 3, __LINE__);
...
const FunctionTestCase fsin_test_cases[] = {
    ...
    // sin(pi/2) = 1
    { { 0x490FDA9E, 128 }, { 0x00000002, 128 } }, // Should be 1, is almost exactly 1
    // sin(pi) = 0
    { { 0x490FDA9E, 129 }, { 0xB1D00000, 109 } }, // Should be 0, is extremely close to 0
    // sin(3pi/2) = -1
    { { 0x16CBE3F7, 130 }, { 0x80000002, 128 } }, // Should be -1, is almost exactly -1
};
...
const FunctionTestCase fcos_test_cases[] = {
    // cos(0) = 1
    { { 0x00000000,   0 }, { 0x00000001, 128 } }, // Should be 1, is almost exactly 1
    // cos(pi/2) = 0
    { { 0x490FDA9E, 128 }, { 0xB1E00000, 108 } }, // Should be 0, is extremely close to 0
    // cos(pi) = -1
    { { 0x490FDA9E, 129 }, { 0xFFFFFFFE, 127 } }, // Should be -1, is almost exactly -1
    // cos(3pi/2) = 0
...
const FunctionTestCase ftan_test_cases[] = {
    ...
    { { 0x00000000, 128 }, { 0x47593DE5, 128 } }, // tan(1.0) = 1.5574077246549022
};
...
const FunctionTestCase fatn_test_cases[] = {
    { { 0x00000000, 0 }, { 0x00000000, 0 } }, // atan(0) = 0.0
        { { 0x00000000, 128 }, { 0x490FDAA3, 127 } }, // atan(1) = 0.7853981633974483
    { { 0x00000000, 127 }, { 0x6D63382D, 126 } }, // atan(0.5) = 0.4636476090008061
    { { 0x00000000, 129 }, { 0x0DB70176, 128 } }, // atan(2) = 1.1071487177940904
    { { 0x80000000, 128 }, { 0xC90FDAA3, 127 } }, // atan(-1) = -0.7853981633974483
};
