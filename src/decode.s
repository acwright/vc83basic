...
decode_string:
        lda     line_pos                ; Add line_pos to line_ptr to get string_ptr
        clc
        adc     line_ptr
        sta     string_ptr
        lda     line_ptr+1
        adc     #0                      ; Leaves carry clear
        sta     string_ptr+1
        ldy     #0
        lda     (string_ptr),y          ; Read string length directly
        adc     #2                      ; Carry is clear: A = length + 2
        adc     line_pos                ; Advance line_pos by length + 2
        sta     line_pos
        rts                                                                                                                                                                         
